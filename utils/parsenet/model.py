...
def knn(x, k1, k2):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            print("Error in knn topk")
            idx = torch.zeros((batch_size, x.shape[2], k1), dtype=torch.long).cuda()
    return idx
...
def knn_points_normals(x, k1, k2):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            print("Error in knn_points_normals topk")
            idx = torch.zeros((batch_size, x.shape[2], k1), dtype=torch.long).cuda()
    return idx
...
def get_graph_feature(x, k1=20, k2=20, idx=None):
    ...
    try:
        ...
        print("Error in get_graph_feature view/index")
        # Return dummy feature to avoid crash
        feature = x.view(batch_size * num_points, -1).repeat(1, k1)[:idx.shape[0], :]
...
def get_graph_feature_with_normals(x, k1=20, k2=20, idx=None):
    ...
    try:
        ...
        print("Error in get_graph_feature_with_normals view/index")
        feature = x.view(batch_size * num_points, -1).repeat(1, k1)[:idx.shape[0], :]
...
class PrimitivesEmbeddingDGCNGn(nn.Module):
    ...
    def forward(self, points, labels=None, compute_loss=False):
        ...
        if compute_loss and labels is not None and self.loss_function is not None:
            embed_loss = self.loss_function(embedding, labels.data.cpu().numpy())
        else:
            embed_loss = torch.zeros(1).cuda() if torch.cuda.is_available() else torch.zeros(1)
